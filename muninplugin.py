...
class AggregateReservedInstanceUtilization(MuninPlugin):
    def __init__(self, region, zones, instance_type):
        title_pretext = instance_type
        ...
        self._cache_file = "/tmp/_cache"
 
    @property
    def fields(self):
        utilization_fields = [self.create_utilization_field(z, self._instance_type) for z in self._zones]
        ri_totals_fields = [self.create_reserved_instance_totals_field(z, self._instance_type) for z in self._zones]
        running_totals_fields = [self.create_running_instance_totals_field(z, self._instance_type) for z in self._zones]
        return utilization_fields + ri_totals_fields + running_totals_fields
 
    def create_utilization_field(self, zone, instance_type):
        warning = -1
        critical = -5
        field = (zone + "_unused_total", dict(
                label = " ".join(["(", zone, ")", "unused", instance_type, "RIs"]),
                info = zone + ' - Number of un-used purchased reserved instances: ' + instance_type,
                draw = "LINE1",
                type = "GAUGE",
                warning = str(warning) + ":",
    ...
    def create_reserved_instance_totals_field(self, zone, instance_type):
        return (zone + "_ri_total", dict(
                label = " ".join(["(", zone, ")", instance_type, "RIs"]),
                info = zone + ' - Number of purchased reserved instances: ' + instance_type,
                draw = "LINE2",
                type = "GAUGE"))
 
    def create_running_instance_totals_field(self, zone, instance_type):
        return (zone + "_running_total", dict(
                label = " ".join(["(", zone, ")", instance_type, "running instances"]),
                info = zone + ' - Number of running instances: ' + instance_type,
                draw = "LINE3",
                type = "GAUGE"))
    ...
    def execute(self):
        ...
        reserved_instance_type_totals =  dict([(z + "_ri_total",ri_totals[z][instance_type])
                                               for z in self._zones
                                               if z in ri_totals.keys() and instance_type in ri_totals[z].keys()])
 
        running_instance_type_totals = dict([(z + "_running_total",running_totals[z][instance_type])
                                             for z in self._zones
                                             if instance_type in running_totals[z].keys()])
        unused_instance_type_totals = dict([(z + "_unused_total", unused_totals[z][instance_type])
                                             for z in self._zones
                                             if instance_type in unused_totals[z].keys()])
 
        utilization_totals = dict([ (z + "_unused_total", actual[region][z][instance_type])
                      for z in actual[region].keys()
                        if instance_type in actual[region][z].keys()])
 
        return dict(reserved_instance_type_totals.items() +
                    utilization_totals.items() +
                    running_instance_type_totals.items() +
                    unused_instance_type_totals.items())
...
if __name__ == "__main__":
    ...
    AggregateReservedInstanceUtilization(region, zones, instance_type).run()
